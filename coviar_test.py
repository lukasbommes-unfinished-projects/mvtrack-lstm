...
for frame_id in range(145):
    ...
    mvs_residuals = np.zeros((residuals.shape[0], residuals.shape[1], 5))
    mvs_residuals[:, :, 0:2] = ret  # XY
    mvs_residuals[:, :, 2:5] = residuals  # BGR

    print("sum", int(np.sum(mvs_residuals)))
    print("is_keyframe", not bool(np.sum(mvs_residuals)))
